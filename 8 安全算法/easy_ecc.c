...
static int getRandomNumber(uint64_t *p_vli)
{
    unsigned char random_gen[ECC_BYTES] = { 0xef,0xcc,0x3e,0x53,0xbc,0x21,0x3e,0x06,0xcb,0xc0,0x6a,0x08,0xd9,0x7c,0xaa,0xc4,0x9e,0xac,0xb9,0x75,0x9c,0x9d,0x08,0x3d,0x3f,0xd7,0x71,0x54,0xbb,0x19,0xc9,0x35 };
    unsigned char temp_buff[ECC_BYTES];
    
    random_gen[0]++;
        if(random_gen[0] == 0xFF)random_gen[1]++;
    simp_sha256_md(random_gen,sizeof(random_gen),(unsigned char*)temp_buff);

    memcpy(p_vli, temp_buff, ECC_BYTES);
    return 1;
}
...
///////////////////* Ó¦ÓÃÓÚECDSA *///////
////////////////////////////////////////////////
int easy_ecc_gen_key(unsigned char p_publicKey[64], unsigned char p_privateKey[32])
{
    uint64_t l_private[NUM_ECC_DIGITS];
    EccPoint l_public;
    unsigned l_tries = 0;
...
int easy_pri_trans_pubkey(const unsigned char* p_privateKey, unsigned char* p_publicKey)
{
    uint64_t l_private[NUM_ECC_DIGITS]; //NUM_ECC_DIGITS = 4
    EccPoint l_public;
...
int easy_ecdsa_verify(const unsigned char p_publicKey[64], const unsigned char p_hash[32], const unsigned char p_signature[64])
{
    unsigned char inner_pub_key[33];

    easy_pub_key_transform(p_publicKey, inner_pub_key);
    return ecdsa_verify(inner_pub_key, p_hash, p_signature);
}

int easy_ecdsa_sign_gen(const unsigned char p_privateKey[32], const unsigned char p_hash[32], unsigned char p_signature[64])
{
    return ecdsa_sign(p_privateKey, p_hash, p_signature);
}

int easy_pub_key_transform(const unsigned char* key_xy, unsigned char* key_xi)
{
    EccPoint* p_ecc_point = (EccPoint*)key_xy;

    memcpy(key_xi + 1, key_xy, ECC_BYTES);
    key_xi[0] = 2 + (p_ecc_point->y[0] & 0x01);
    return 0;
}

int easy_ecdh_shared_secret(const unsigned char p_publicKey[64], const unsigned char p_privateKey[32], unsigned char p_secret[32])
{
    unsigned char t_pub_key[33];
    easy_pub_key_transform(p_publicKey, t_pub_key);
    return ecdh_shared_secret(t_pub_key, p_privateKey, p_secret);
}
